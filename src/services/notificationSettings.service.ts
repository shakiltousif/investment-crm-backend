...
// Type assertion for Prisma client to include in-app notification settings model
...
export interface InAppNotificationSettingsData {
...
export class NotificationSettingsService {
  /**
   * Get in-app notification settings for a user (or global if userId is null)
   */
  async getSettings(userId?: string | null): Promise<InAppNotificationSettingsData> {
    // Handle null userId for global settings - use findFirst for null values
    let settings;
    if (userId === null || userId === undefined) {
      settings = (await prismaClient.inAppNotificationSettings.findFirst({
        where: { userId: null },
      })) as Record<string, unknown> | null;
    } else {
      settings = (await prismaClient.inAppNotificationSettings.findUnique({
        where: { userId },
      })) as Record<string, unknown> | null;
    }
    ...
    const settingsData = settings as InAppNotificationSettingsData;
    return {
  ...
  /**
   * Update in-app notification settings for a user (or global if userId is null)
   */
  async updateSettings(
    userId: string | null,
    data: InAppNotificationSettingsData
  ): Promise<InAppNotificationSettingsData> {
    ...
    if (userIdValue === null) {
      ...
      const existing = (await prismaClient.inAppNotificationSettings.findFirst({
        where: { userId: null },
      })) as Record<string, unknown> | null;

      if (existing) {
        settings = (await prismaClient.inAppNotificationSettings.update({
          where: { id: (existing as { id: string }).id },
          data: updateData,
        })) as Record<string, unknown>;
      } else {
        settings = (await prismaClient.inAppNotificationSettings.create({
          data: {
            userId: null,
      ...
      settings = (await prismaClient.inAppNotificationSettings.upsert({
  ...
  async shouldSendNotification(
    userId: string | null,
    notificationType: keyof InAppNotificationSettingsData
  ): Promise<boolean> {
